...
public enum CliOptions {
    ...
    private static OutputConfig selectPredefinedJsonConfig(CommandLine cmd) {
        ...
        if (cmd.hasOption(CONFIG.shortName)) {
            try {
                try (InputStream r = CliOptions.class.getResourceAsStream(cmd.getOptionValue(CONFIG.shortName))) {
                    if (r != null) {
                        result = m.readValue(r, OutputConfig.class);
                    } else {
            ...
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (cmd.hasOption(ANDROID_SMALL.longName)) {
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android-small.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (cmd.hasOption(ANDROID_ICON.longName)) {
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android-icon.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (cmd.hasOption(ANDROID_LAUNCH.longName)) {
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android-launcher.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (cmd.hasOption(ANDROID_24dp.longName)) {
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android-24dp.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (cmd.hasOption(ANDROID_36dp.longName)) {
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android-36dp.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (cmd.hasOption(ANDROID_48dp.longName)) {
            try {
                result = m.readerFor(OutputConfig.class).readValue(OutputConfig.class.getResourceAsStream("/android-48dp.json"));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
